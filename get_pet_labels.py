...
def get_pet_labels(image_dir):
    ...
    for idx in range(0, len(in_files), 1):
       ...
       if in_files[idx][0] != ".":
           ...
           pet_label = extract_name(file_name)
...
def extract_name(full_file_name):
  name = ""
  words_in_file_name = full_file_name.split("_")
  for word in words_in_file_name:    
    if word.isalpha(): 
      name += word + " "    
  return name.strip().lower()

if __name__ == "__main__":
  pet_name_dict = get_pet_labels("pet_images/")
  print(pet_name_dict)
